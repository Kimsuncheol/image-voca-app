const ko = {
  common: {
    back: "뒤로",
    cancel: "취소",
    close: "닫기",
    upgrade: "업그레이드",
    error: "오류",
    info: "정보",
    success: "성공",
    notice: "알림",
    confirm: "확인",
    or: "또는",
    words: "단어",
    restart: "다시하기",
    finish: "완료",
    submit: "제출",
    next: "다음",
    retry: "다시하기",
    loading: "로딩중...",
  },
  tabs: {
    dashboard: "대시보드",
    wordBank: "단어장",
    voca: "단어",
    speaking: "스피킹",
    settings: "설정",
  },
  settings: {
    title: "설정",
    appearance: {
      title: "화면",
      light: "라이트",
      dark: "다크",
      system: "시스템",
    },
    notifications: {
      title: "알림",
      push: "푸시 알림",
      studyReminder: "학습 알림",
      wordOfTheDay: "오늘의 단어",
      notSupportedTitle: "지원되지 않음",
      notSupportedMessage:
        "Android Expo Go에서는 푸시 알림을 지원하지 않습니다.",
      moduleMissing: "알림 모듈을 불러오지 못했습니다.",
      permissionTitle: "권한 필요",
      permissionMessage: "시스템 설정에서 알림을 허용해주세요.",
      openSettings: "설정 열기",
    },
    account: {
      title: "계정",
      profile: "프로필",
      billing: "요금제 및 결제",
      signOut: "로그아웃",
      signOutError: "로그아웃 실패",
    },
    language: {
      title: "언어",
      english: "영어",
      korean: "한국어",
    },
    admin: {
      title: "관리자",
      addVocabulary: "단어 추가",
      advertisements: "광고 관리",
      members: "회원 관리",
      promotionCodes: "프로모션 코드",
      adminCodes: "관리자 코드",
    },
    teacher: {
      title: "교사",
      classes: "클래스 관리",
      students: "전체 학생 보기",
      assignments: "과제 관리",
      analytics: "분석 보기",
    },
  },
  dashboard: {
    greeting: {
      morning: "좋은 아침이에요",
      afternoon: "좋은 오후예요",
      evening: "좋은 저녁이에요",
    },
    fallbackUser: "학습자",
    menu: {
      profile: "프로필",
      friends: "친구",
      leaderboard: "리더보드",
    },
    progressComplete: "{{percent}}% 완료",
    popQuiz: {
      title: "팝 퀴즈",
      headline: "빠르게 실력을 확인해볼까요?",
      subtitle: "짧은 퀴즈로 기억을 테스트하세요.",
      wordLabel: "오늘의 단어",
      question: "이 단어의 뜻은?",
      correct: "정답입니다!",
      incorrect: "내일 다시 도전해보세요.",
    },
    quickActions: {
      title: "빠른 실행",
      vocabulary: "단어 학습",
      wordBank: "단어장",
      speaking: "스피킹",
      settings: "설정",
      friends: "친구",
      leaderboard: "리더보드",
    },
    todayProgress: "오늘의 진행률",
    dailyGoal: "일일 목표",
    stats: {
      title: "통계",
      wordsLearned: "학습한 단어",
      thisWeek: "이번 주",
      streak: "연속 학습",
      days: "일",
      accuracy: "정확도",
      last7Days: "최근 7일",
      timeSpent: "학습 시간",
    },
    friendActivity: {
      title: "친구",
      seeAll: "전체보기",
      newRequests: "새로운 친구 요청",
      tapToView: "탭하여 확인 및 응답",
    },
  },
  friends: {
    title: "친구",
    myFriends: "친구",
    received: "받은 요청",
    sent: "보낸 요청",
    search: "검색",
    searchPlaceholder: "이름 또는 이메일로 검색",
    noFriends: "아직 친구가 없습니다. 사용자를 검색하여 친구를 추가하세요!",
    noRequestsReceived: "받은 친구 요청이 없습니다",
    noRequestsSent: "보낸 친구 요청이 없습니다",
    noUsersFound: "사용자를 찾을 수 없습니다",
    pending: "대기중",
    success: "성공",
    error: "오류",
    requestSent: "친구 요청을 보냈습니다!",
    requestAccepted: "친구 요청을 수락했습니다!",
    requestFailed: "친구 요청을 보내지 못했습니다",
    acceptFailed: "친구 요청 수락에 실패했습니다",
    rejectFailed: "친구 요청 거절에 실패했습니다",
    removeFriend: "친구 삭제",
    removeFriendConfirm: "{{name}}님을 친구 목록에서 삭제하시겠습니까?",
    remove: "삭제",
    friendRemoved: "친구가 삭제되었습니다",
    removeFailed: "친구 삭제에 실패했습니다",
    cancelRequest: "요청 취소",
    cancelRequestConfirm: "이 친구 요청을 취소하시겠습니까?",
    cancelButton: "요청 취소",
    cancelFailed: "요청 취소에 실패했습니다",
    profile: "프로필",
    lastActive: "마지막 활동",
    loadProfileFailed: "프로필을 불러오지 못했습니다",
  },
  leaderboard: {
    title: "리더보드",
    noEntries: "아직 항목이 없습니다",
  },
  alerts: {
    premiumFeature: {
      title: "프리미엄 기능",
      message: "스피킹 연습은 Voca + Speaking 요금제가 필요합니다.",
    },
  },
  wordBank: {
    title: "단어장",
    subtitle: "코스별 저장한 단어",
    wordsTitle: "단어",
    empty: {
      title: "저장된 단어가 없습니다",
      subtitle: '단어 카드를 넘기며 "단어장에 추가"를 눌러 저장하세요.',
    },
  },
  courses: {
    vocabularyTitle: "단어",
    selectCourse: "학습할 코스를 선택하세요",
    recentCourse: "최근 코스",
    allCourses: "전체 코스",
    recent: "최근",
    wordCount: "{{count}} 단어",
    wordCount_one: "{{count}} 단어",
    wordCount_other: "{{count}} 단어",
    csat: {
      title: "수능",
      description: "한국 대학수학능력시험",
    },
    toeic: {
      title: "TOEIC",
      description: "비즈니스 영어",
    },
    toefl: {
      title: "TOEFL",
      description: "학문 영어",
    },
    toeicSpeaking: {
      title: "TOEIC Speaking",
      description: "스피킹 스킬",
    },
    ielts: {
      title: "IELTS",
      description: "국제 영어",
    },
    opic: {
      title: "OPIc",
      description: "영어 말하기 시험",
    },
  },
  subscription: {
    unlimited: {
      title: "무제한 단어 학습",
      description: "Voca Unlimited로 업그레이드해 무제한으로 학습하세요.",
    },
    speaking: {
      title: "스피킹 연습",
      description: "Voca + Speaking으로 업그레이드해 스피킹을 시작하세요.",
    },
    upgradeHint: "업그레이드하여 더 많은 기능을 이용하세요",
    status: {
      active: "활성",
    },
  },
  plans: {
    free: {
      name: "무료",
      features: ["제한된 단어 학습", "스피킹 연습 잠김"],
    },
    voca_unlimited: {
      name: "Voca Unlimited",
      features: ["무제한 단어 학습", "스피킹 연습 잠김"],
    },
    voca_speaking: {
      name: "Voca + Speaking Unlimited",
      features: ["무제한 단어 학습", "스피킹 연습 해제"],
    },
  },
  home: {
    welcome: "환영합니다!",
  },
  swipe: {
    actions: {
      adding: "추가 중...",
      added: "추가됨",
      addToWordBank: "단어장에 추가",
    },
    errors: {
      loginRequired: "단어를 저장하려면 로그인하세요.",
      alreadyAdded: "이미 단어장에 있는 단어입니다.",
      addFailed: "단어장에 추가하지 못했습니다.",
    },
    success: {
      addedToWordBank: '"{{word}}"를 단어장에 추가했습니다!',
    },
  },
  auth: {
    errors: {
      missingCredentials: "이메일과 비밀번호를 모두 입력하세요.",
      loginTitle: "로그인 오류",
      permissionTitle: "권한 필요",
      permissionMessage: "사진 라이브러리 접근 권한이 필요합니다.",
      missingFields: "모든 항목을 입력하세요.",
      passwordRequirements: "비밀번호 요구 사항을 충족하세요.",
      passwordMismatch: "비밀번호가 일치하지 않습니다.",
      registerTitle: "회원가입 오류",
      invalidEmail: "유효한 이메일 주소를 입력하세요",
      invalidAdminCode: "잘못된 관리자 코드",
    },
    login: {
      title: "다시 오신 것을 환영합니다",
      subtitle: "계속하려면 로그인하세요",
      emailPlaceholder: "이메일",
      passwordPlaceholder: "비밀번호",
      rememberMe: "로그인 상태 유지",
      forgotPassword: "비밀번호를 잊으셨나요?",
      signingIn: "로그인 중...",
      signIn: "로그인",
      googleSigningIn: "로그인 중...",
      googleSignIn: "Google로 로그인",
      noAccount: "계정이 없나요? ",
      signUp: "회원가입",
    },
    register: {
      title: "계정 만들기",
      subtitle: "시작하려면 회원가입하세요",
      avatarLabel: "프로필 사진 추가(선택)",
      fullNamePlaceholder: "이름",
      emailPlaceholder: "이메일",
      passwordPlaceholder: "비밀번호",
      confirmPasswordPlaceholder: "비밀번호 확인",
      passwordHint: {
        length: "8자 이상",
        number: "숫자 포함",
        special: "특수문자 포함",
        match: "비밀번호 일치",
      },
      adminCodeLabel: "관리자 코드 (선택사항)",
      adminCodePlaceholder: "ADMIN-XXX-YYY",
      validAdminCode: "유효한 관리자 코드 - 관리자로 등록됩니다",
      creatingAccount: "계정 생성 중...",
      register: "회원가입",
      googleSigningIn: "로그인 중...",
      googleSignIn: "Google로 로그인",
      hasAccount: "이미 계정이 있나요? ",
      signIn: "로그인",
    },
  },
  profile: {
    title: "프로필",
    userFallback: "사용자",
    unsaved: {
      title: "변경 사항을 버릴까요?",
      message: "저장되지 않은 변경 사항이 있습니다. 화면을 떠나시겠습니까?",
      stay: "머무르기",
      discard: "버리기",
    },
    permission: {
      title: "권한 필요",
      message: "사진 라이브러리 접근 권한이 필요합니다!",
    },
    save: {
      success: "프로필이 업데이트되었습니다!",
      error: "프로필 업데이트 실패: {{message}}",
      saving: "저장 중...",
      saveChanges: "변경 사항 저장",
    },
    delete: {
      title: "계정 삭제",
      message: "계정을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.",
      confirm: "삭제",
      confirmButton: "삭제 확인",
      securityTitle: "보안 확인",
      securityMessage: "계정 삭제를 위해 비밀번호를 다시 입력하세요.",
      passwordRequired: "비밀번호를 입력하세요.",
      failed: "계정 삭제에 실패했습니다. 비밀번호를 확인하고 다시 시도하세요.",
      processing: "처리 중...",
      confirmPassword: "비밀번호 확인",
      passwordPlaceholder: "비밀번호",
    },
    sections: {
      accountInfo: "계정 정보",
      learningGoals: "학습 목표",
      accountActions: "계정 작업",
    },
    fields: {
      name: "이름",
      displayNamePlaceholder: "표시 이름",
      email: "이메일",
      role: "역할",
      dailyGoal: "일일 목표",
      wordsPerDay: "단어/일",
    },
    goal: {
      updated: "일일 목표가 {{goal}}단어로 업데이트되었습니다!",
      invalid: "유효한 숫자를 입력하세요.",
      updateButton: "목표 업데이트",
    },
  },
  billing: {
    title: "요금제",
    selectTitle: "요금제 선택",
    selectSubtitle: "학습 목표에 맞는 요금제를 선택하세요",
    currentPlan: "현재 요금제: {{plan}}",
    recommended: "추천",
    oneTimePayment: "일회성 결제",
    currentPlanButton: "현재 요금제",
    freePlanButton: "기본 요금제",
    purchaseButton: "구매하기",
    info: "일회성 결제로 평생 이용 가능합니다. 환불은 구매 후 7일 이내에만 가능합니다.",
    checkout: {
      title: "결제",
      invalidPlan: "잘못된 요금제입니다.",
      loginRequired: "로그인이 필요합니다.",
      notInitialized: "결제 정보가 초기화되지 않았습니다.",
      agreeRequired: "필수 약관에 동의해주세요.",
      orderName: "{{plan}} 요금제",
      successTitle: "결제 완료",
      successMessage: "{{plan}} 요금제가 활성화되었습니다.",
      failTitle: "결제 실패",
      failMessage: "결제에 실패했습니다.",
      errorMessage: "결제 처리 중 오류가 발생했습니다.",
      summaryTitle: "주문 정보",
      productLabel: "상품명",
      amountLabel: "결제 금액",
      featuresTitle: "포함된 기능",
      paymentMethods: "결제 수단",
      processing: "결제 처리 중...",
      payButton: "{{price}} 결제하기",
      securityNote: "토스페이먼츠 보안 결제",
    },
  },
  promotion: {
    section: {
      title: "프로모션 코드가 있으신가요?",
    },
    input: {
      placeholder: "코드 입력",
    },
    button: {
      redeem: "코드 사용하기",
    },
    helper: {
      format: "8자리 프로모션 코드를 입력하세요",
    },
    success: {
      title: "성공!",
      message: "프로모션 코드가 성공적으로 적용되었습니다.",
    },
    banner: {
      title: "특별 프로모션!",
      subtitle: "아래에서 코드를 입력하세요",
      expired: "만료됨",
      daysRemaining: "{{days}}일 남음",
      daysRemaining_one: "{{count}}일 남음",
      daysRemaining_other: "{{count}}일 남음",
      hoursRemaining: "{{hours}}시간 남음",
      hoursRemaining_one: "{{count}}시간 남음",
      hoursRemaining_other: "{{count}}시간 남음",
      minutesRemaining: "{{minutes}}분 남음",
      minutesRemaining_one: "{{count}}분 남음",
      minutesRemaining_other: "{{count}}분 남음",
    },
  },
  course: {
    days: "일차",
    selectDay: "학습할 일차를 선택하세요",
    dayTitle: "{{day}}일차",
    dayCompleted: "오늘 학습 완료!",
    takeQuiz: "퀴즈 풀기",
    progress: "{{learned}}/{{total}} 단어",
    retake: "정복 완료",
    quiz: "퀴즈",
    checked: "확인됨",
    premiumLimitMessage:
      "{{day}}일차 이후 학습은 Voca Unlimited 요금제가 필요합니다.",
  },
  quiz: {
    typeTitle: "퀴즈 유형",
    selectType: "퀴즈 유형 선택",
    courseDaySubtitle: "{{course}} - {{day}}일차",
    types: {
      multipleChoice: {
        title: "객관식",
        description: "정답 의미 선택",
      },
      fillBlank: {
        title: "빈칸 채우기",
        description: "정답 단어 입력",
      },
      fillInBlank: {
        title: "빈칸 채우기",
        description: "문장을 완성하세요",
        targetWord: "목표 단어",
        completeSentence: "문장을 완성하세요",
        chooseWord: "정답 단어를 선택하세요",
      },
      matching: {
        title: "매칭",
        description: "단어와 뜻을 짝지으세요",
      },
      listening: {
        title: "리스닝",
        description: "듣고 단어 입력",
      },
      spelling: {
        title: "스펠링",
        description: "정확한 철자를 입력하세요",
      },
      flashcard: {
        title: "플래시카드 복습",
        description: "빠른 복습 모드",
      },
      wordArrangement: {
        title: "문장 배열",
        description: "단어를 배열하여 문장을 만드세요",
      },
    },
    questionTitle: "{{current}}/{{total}}문항",
    questions: {
      matchMeaning: "이 뜻에 맞는 단어는?",
      spellPrompt: "뜻에 맞는 단어의 철자를 입력하세요.",
      meaningOf: "다음 단어의 뜻은?",
    },
    actions: {
      typeAnswer: "정답을 입력하세요...",
      typeSpelling: "철자를 입력하세요...",
    },
    matching: {
      instructions: "단어와 뜻을 각각 눌러 짝을 맞추세요.",
      progressTitle: "매칭 {{current}}/{{total}}",
    },
    wordArrangement: {
      instructions: "올바른 문장을 만들려면 단어를 순서대로 탭하세요.",
      targetWord: "대상 단어",
      yourSentence: "내 문장",
      tapToRemove: "탭하여 제거",
      completed: "정답! 🎉",
      noExample: "예문이 없습니다.",
    },
    feedback: {
      correct: "정답!",
      incorrect: "오답",
      correctAnswer: "정답: {{answer}}",
    },
    results: {
      title: "결과",
      excellent: "훌륭해요! 🎉",
      goodJob: "잘했어요! 👍",
      keepPracticing: "계속 연습해요! 💪",
    },
    seeResults: "결과 보기",
  },
  admin: {
    members: {
      title: "회원 관리",
      accessDenied: "접근 거부",
      noPermission: "이 페이지에 접근할 권한이 없습니다.",
      checkingPermissions: "권한 확인 중...",
      loadError: "회원 목록을 불러오지 못했습니다",
      detailsError: "회원 정보를 불러오지 못했습니다",
      updateError: "회원 정보를 업데이트하지 못했습니다",
      roleUpdated: "역할이 업데이트되었습니다",
      planUpdated: "구독이 업데이트되었습니다",
      totalMembers: "전체",
      unlimited: "Unlimited",
      speaking: "Speaking",
      searchPlaceholder: "이름 또는 이메일로 검색...",
      filter: "필터",
      allPlans: "모든 요금제",
      adminsOnly: "관리자만",
      members: "회원",
      noResults: "검색 결과가 없습니다",
      noMembers: "회원이 없습니다",
      neverActive: "활동 없음",
      memberDetails: "회원 정보",
      role: "역할",
      subscription: "구독",
      activatedAt: "활성화 일시",
      activatedBy: "활성화 방법",
      stats: "통계",
      currentStreak: "현재 연속",
      longestStreak: "최장 연속",
      wordsLearned: "학습한 단어",
      accuracy: "정확도",
      activity: "활동",
      lastActive: "마지막 활동",
      dailyGoal: "일일 목표",
    },
  },
  speaking: {
    title: "스피킹 연습",
    header: {
      title: "스피킹 연습",
      subtitle: "AI 피드백과 함께 스피킹 실력을 향상시키세요",
    },
    toeic: {
      title: "TOEIC Speaking",
      description: "TOEIC Speaking 시험 6개 파트 연습",
      practice: "연습",
      results: "결과",
      selectParts: "연습할 파트 선택",
      prepTime: "준비: {{time}}초",
      responseTime: "응답: {{time}}초",
      startPractice: "연습 시작 ({{count}}개 파트)",
      selectToStart: "시작하려면 파트를 선택하세요",
      generating: "문제 생성 중...",
      beginQuestion: "문제 시작",
      skipPrep: "준비 건너뛰기",
      parts: {
        1: {
          title: "문장 읽기",
          description: "텍스트를 명확하게 소리내어 읽기",
        },
        2: {
          title: "문장 읽기",
          description: "텍스트를 명확하게 소리내어 읽기",
        },
        3: { title: "사진 묘사", description: "사진을 상세하게 설명하기" },
        4: {
          title: "질문 응답",
          description: "일상 주제에 대한 질문에 답하기",
        },
        5: { title: "해결책 제시", description: "문제를 듣고 해결책 제안하기" },
        6: { title: "의견 표현", description: "주제에 대한 의견 말하기" },
      },
    },
    opic: {
      title: "OPIc",
      description: "AI 레벨 예측과 함께하는 인터뷰 연습",
      practice: "연습",
      results: "결과",
      targetLevel: "목표 레벨",
      selectTopics: "주제 선택",
      questionsCount: "{{count}}개 질문",
      startPractice: "연습 시작 ({{count}}개 주제)",
      selectToStart: "시작하려면 주제를 선택하세요",
      preparing: "인터뷰 준비 중...",
      startAnswer: "답변 시작",
      readyNow: "준비됨",
      predictedLevel: "예상 레벨",
      topics: {
        selfIntro: "자기소개",
        hobbies: "취미 및 관심사",
        travel: "여행",
        technology: "기술",
        food: "음식 및 요리",
      },
      levels: {
        IL: "Intermediate Low - 기본적인 대화 능력",
        IM1: "Intermediate Mid 1 - 간단한 상황 처리 가능",
        IM2: "Intermediate Mid 2 - 익숙한 주제에서 편안함",
        IM3: "Intermediate Mid 3 - 확장된 대화 능력",
        IH: "Intermediate High - 복잡한 상황 처리 가능",
        AL: "Advanced Low - 전문적 수준의 의사소통",
      },
    },
    recording: "녹음 중...",
    stopRecording: "녹음 중지",
    finishAnswer: "답변 완료",
    analyzing: "응답 분석 중...",
    timer: {
      prep: "준비 시간",
      response: "응답 시간",
      thinking: "생각",
      speaking: "말하기",
    },
    categories: {
      pronunciation: "발음",
      fluency: "유창성",
      grammar: "문법",
      vocabulary: "어휘",
      content: "내용",
    },
    results: {
      excellent: "훌륭합니다! 🎉",
      good: "잘했어요! 👍",
      needsPractice: "계속 연습하세요! 💪",
      question: "질문 {{num}}",
      yourResponse: "내 응답",
      strengths: "강점",
      suggestions: "개선할 점",
    },
    features: {
      title: "기능",
      aiAnalysis: "AI 음성 분석",
      scoring: "상세 점수 분석",
      feedback: "맞춤형 피드백",
      timedPractice: "시간 제한 연습",
    },
    locked: {
      title: "프리미엄 기능",
      message: "스피킹 연습은 Voca + Speaking 요금제가 필요합니다.",
      upgrade: "요금제 보기",
    },
    notice: {
      premium: "AI 스피킹 연습을 잠금 해제하려면 업그레이드하세요",
    },
    errors: {
      permissionTitle: "마이크 권한 필요",
      permissionMessage: "응답을 녹음하려면 마이크 접근을 허용해주세요.",
      recordingFailed: "녹음을 시작할 수 없습니다. 다시 시도해주세요.",
    },
  },
  adminCodes: {
    title: "관리자 코드 관리",
    subtitle: "관리자 등록 코드 생성 및 관리",
    generate: {
      title: "새 코드 생성",
      maxUses: "최대 사용 횟수",
      maxUsesPlaceholder: "최대 사용 횟수 입력 (무제한은 -1)",
      expiresInDays: "만료 기간 (일)",
      expiresInDaysPlaceholder: "일수 입력 (선택사항)",
      description: "설명",
      descriptionPlaceholder: "내부 메모 (선택사항)",
      generateButton: "코드 생성",
      generating: "생성 중...",
      success: "관리자 코드가 성공적으로 생성되었습니다!",
      error: "관리자 코드 생성 실패",
    },
    list: {
      title: "활성 코드",
      emptyTitle: "관리자 코드가 없습니다",
      emptyMessage: "첫 번째 관리자 코드를 생성하세요",
      code: "코드",
      uses: "사용 횟수",
      expires: "만료",
      created: "생성일",
      status: "상태",
      active: "활성",
      inactive: "비활성",
      expired: "만료됨",
      unlimited: "무제한",
      never: "없음",
      deactivate: "비활성화",
      copy: "복사",
      copied: "복사됨!",
    },
    stats: {
      total: "전체 코드",
      active: "활성",
      used: "총 사용 횟수",
    },
  },
  teacher: {
    classes: {
      title: "내 클래스",
    },
    classDetail: {
      title: "클래스 상세",
    },
    studentProfile: {
      title: "학생 프로필",
    },
    createAssignment: {
      title: "과제 생성",
    },
    editAssignment: {
      title: "과제 수정",
    },
    assignmentDetail: {
      title: "과제 상세",
    },
    classSettings: {
      title: "클래스 설정",
      basicInfo: "기본 정보",
      allowSelfEnrollment: "자체 등록 허용",
      allowSelfEnrollmentDesc: "학생들이 초대 코드로 참여할 수 있습니다",
      dailyGoal: "일일 목표 (단어)",
      dailyGoalPlaceholder: "일일 목표 입력",
      dailyGoalHint: "선택사항: 학생들을 위한 일일 단어 학습 목표 설정",
      loadError: "클래스 설정을 불러오지 못했습니다",
      saveSuccess: "클래스 설정이 업데이트되었습니다",
      saveError: "클래스 설정 저장 실패",
      archiveConfirm: {
        title: "클래스 보관",
        message: "이 클래스를 보관하시겠습니까? 학생들은 더 이상 접근할 수 없습니다.",
        confirm: "보관",
      },
      archiveSuccess: "클래스가 보관되었습니다",
      archiveError: "클래스 보관 실패",
      validation: {
        nameRequired: "클래스 이름이 필요합니다",
      },
    },
    students: {
      title: "학생",
      sortBy: "정렬 기준",
      sortByName: "이름",
      sortByProgress: "진행률",
      sortByStreak: "연속 학습",
      noStudents: "아직 이 클래스에 학생이 없습니다",
    },
    permission: {
      denied: "교사 전용",
    },
    dashboard: {
      title: "교사 대시보드",
      welcome: "환영합니다",
      myClasses: "내 클래스",
      createClass: "클래스 생성",
      noClasses: "클래스가 없습니다",
      noClassesMessage: "첫 번째 클래스를 생성하여 시작하세요",
      studentsCount: "{{count}}명",
      studentsCount_one: "{{count}}명",
      studentsCount_other: "{{count}}명",
      coursesCount: "{{count}}개 코스",
      coursesCount_one: "{{count}}개 코스",
      coursesCount_other: "{{count}}개 코스",
    },
    class: {
      create: {
        title: "클래스 생성",
        className: "클래스 이름",
        classNamePlaceholder: "클래스 이름 입력",
        description: "설명",
        descriptionPlaceholder: "클래스 설명 입력 (선택사항)",
        selectCourses: "코스 선택",
        selectCoursesHint: "학생들이 접근할 수 있는 코스를 선택하세요",
        creating: "생성 중...",
        createButton: "클래스 생성",
        success: "클래스가 성공적으로 생성되었습니다!",
        error: "클래스 생성 실패",
        validationError: "클래스 이름을 입력하고 최소 하나의 코스를 선택하세요",
      },
      detail: {
        title: "클래스 상세",
        students: "학생",
        assignments: "과제",
        analytics: "분석",
        settings: "설정",
        inviteCode: "초대 코드",
        inviteCodeCopied: "초대 코드가 복사되었습니다!",
        courses: "코스",
        description: "설명",
        noDescription: "설명이 없습니다",
        createdOn: "생성일",
        archive: "클래스 보관",
        archiveConfirm: "이 클래스를 보관하시겠습니까? 학생들의 접근 권한이 사라집니다.",
        archiveSuccess: "클래스가 보관되었습니다",
        archiveError: "클래스 보관 실패",
        loading: "클래스 정보 로딩 중...",
        notFound: "클래스를 찾을 수 없습니다",
        accessDenied: "이 클래스를 볼 권한이 없습니다",
      },
      students: {
        title: "학생",
        addStudent: "학생 추가",
        removeStudent: "학생 제거",
        removeConfirm: "{{name}}을(를) 이 클래스에서 제거하시겠습니까?",
        removeSuccess: "학생이 제거되었습니다",
        removeError: "학생 제거 실패",
        noStudents: "등록된 학생이 없습니다",
        noStudentsMessage: "학생들에게 초대 코드를 공유하여 참여하도록 하세요",
        lastActive: "마지막 활동: {{date}}",
        neverActive: "활동 없음",
      },
    },
    assignments: {
      title: "과제",
      create: {
        title: "과제 생성",
        selectCourse: "코스 선택",
        selectCoursePlaceholder: "코스를 선택하세요",
        dayNumber: "일차",
        dayNumberPlaceholder: "일차 입력",
        assignmentTitle: "제목",
        assignmentTitlePlaceholder: "과제 제목 입력",
        description: "설명",
        descriptionPlaceholder: "설명 입력 (선택사항)",
        dueDate: "마감일",
        requirements: "요구사항",
        requireVocabulary: "단어 학습 완료",
        requireQuiz: "퀴즈 완료",
        minQuizScore: "최소 퀴즈 점수 (%)",
        minQuizScorePlaceholder: "최소 점수 입력 (선택사항)",
        creating: "생성 중...",
        createButton: "과제 생성",
        success: "과제가 성공적으로 생성되었습니다!",
        error: "과제 생성 실패",
        validationError: "모든 필수 항목을 입력하세요",
      },
      list: {
        title: "과제",
        createNew: "과제 생성",
        noAssignments: "과제가 없습니다",
        noAssignmentsMessage: "첫 번째 과제를 생성하여 학생 진행 상황을 추적하세요",
        dueDate: "마감: {{date}}",
        overdue: "{{days}}일 지남",
        dueToday: "오늘 마감",
        dueIn: "{{days}}일 남음",
        submitted: "{{count}}/{{total}} 제출",
        requiresVocabulary: "단어",
        requiresQuiz: "퀴즈",
        minScore: "최소 {{score}}%",
      },
      detail: {
        title: "과제 상세",
        course: "코스",
        day: "일차",
        dueDate: "마감일",
        requirements: "요구사항",
        completeVocabulary: "단어 학습 완료",
        completeQuiz: "퀴즈 완료",
        minQuizScore: "최소 퀴즈 점수: {{score}}%",
        description: "설명",
        noDescription: "설명이 없습니다",
        submissions: "제출",
        submissionStats: "{{completed}}/{{total}} 완료",
        noSubmissions: "제출이 없습니다",
        deleteAssignment: "과제 삭제",
        deleteConfirm: "이 과제를 삭제하시겠습니까?",
        deleteSuccess: "과제가 삭제되었습니다",
        deleteError: "과제 삭제 실패",
        loading: "과제 로딩 중...",
        notFound: "과제를 찾을 수 없습니다",
        accessDenied: "이 과제를 볼 권한이 없습니다",
      },
      status: {
        pending: "대기중",
        completed: "완료",
        overdue: "기한 지남",
        submitted: "제출됨",
      },
    },
    analytics: {
      title: "분석",
      overview: {
        title: "개요",
        totalStudents: "전체 학생",
        activeStudents: "활동 중인 학생",
        totalClasses: "전체 클래스",
        activeClasses: "활동 중인 클래스",
        completionRate: "완료율",
        avgQuizScore: "평균 퀴즈 점수",
      },
      period: {
        week: "주간",
        month: "월간",
        selectPeriod: "기간 선택",
      },
      metrics: {
        activeStudents: "활동 학생",
        totalWords: "총 단어",
        avgAccuracy: "평균 정확도",
        timeSpent: "학습 시간",
        hours: "시간",
      },
      topPerformers: {
        title: "우수 학생",
        noData: "성과 데이터가 없습니다",
        wordsLearned: "{{count}}개 단어",
        rank: "#{rank}",
      },
      needsAttention: {
        title: "주의가 필요한 학생",
        noAlerts: "모든 학생이 잘하고 있습니다!",
        severity: {
          high: "높음",
          medium: "중간",
          low: "낮음",
        },
        alertTypes: {
          inactive: "비활성",
          low_performance: "낮은 성적",
          streak_lost: "연속 학습 끊김",
        },
      },
      trendData: {
        title: "활동 추이",
        noData: "추이 데이터가 없습니다",
      },
      loading: "분석 로딩 중...",
      error: "분석 로드 실패",
      refresh: "당겨서 새로고침",
    },
    settings: {
      title: "교사 설정",
      classManagement: "클래스 관리",
      viewClasses: "클래스 보기",
      createClass: "새 클래스 생성",
      analytics: "분석",
      viewAnalytics: "분석 보기",
      support: "지원",
      helpCenter: "도움말 센터",
      contactSupport: "고객 지원 문의",
    },
  },
  student: {
    classes: {
      title: "내 클래스",
      joinClass: "클래스 참여",
      noClasses: "아직 등록된 클래스가 없습니다",
      noClassesMessage: "선생님에게 초대 코드를 받아 클래스에 참여하세요",
      joinModal: {
        title: "클래스 참여",
        inviteCode: "초대 코드",
        inviteCodePlaceholder: "6자리 코드 입력",
        joining: "참여 중...",
        joinButton: "클래스 참여",
        cancel: "취소",
        success: "클래스에 성공적으로 참여했습니다!",
        error: "클래스 참여 실패",
        invalidCode: "유효한 6자리 코드를 입력하세요",
      },
      card: {
        teacher: "교사: {{name}}",
        students: "{{count}}명",
        courses: "{{count}}개 코스",
      },
    },
    assignments: {
      title: "내 과제",
      dueCard: {
        dueIn: "{{days}}일 남음",
        dueToday: "오늘 마감",
        overdue: "기한 지남",
        completed: "완료",
        pending: "대기중",
      },
      noAssignments: "과제가 없습니다",
      noAssignmentsMessage: "선생님이 아직 과제를 부여하지 않았습니다",
      completionStatus: {
        vocabularyComplete: "단어 학습 완료",
        quizComplete: "퀴즈 완료",
        vocabularyPending: "단어 학습 필요",
        quizPending: "퀴즈 완료 필요",
        scoreRequired: "{{score}}% 이상 득점 필요",
      },
    },
  },
};

export default ko;
