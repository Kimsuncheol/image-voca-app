...
export default function OPIcPracticeScreen() {
  const { isDark } = useTheme();
  const { t } = useTranslation();
  const router = useRouter();
  const { level, topics } = useLocalSearchParams<{
    level: OPIcLevel;
    topics: string;
  }>();
  ...
  useEffect(() => {
    const topicIds = topics?.split(",") || ["self-intro"];
    startOPIcSession(level || "IM2", topicIds);
    ...
    try {
      ...
      if (uri) {
        ...
        if (progress.current >= progress.total) {
          completeSession();
          router.replace("/speaking/opic/results");
        } else {
          nextQuestion();
        }
      }
        ...
        <View style={styles.topicBadgeContainer}>
          <View style={styles.topicBadge}>
            <ThemedText style={styles.topicBadgeText}>
              {t(currentQuestion.topic.titleKey)}
            </ThemedText>
          </View>
        </View>
      )}
        ...
        <View style={styles.timerContainer}>
          ...
          >
            ...
            <ThemedText style={styles.timerLabel}>
              {timerMode === "prep"
                ? t("speaking.timer.thinking")
                : t("speaking.timer.speaking")}
            </ThemedText>
          </View>
        </View>
      )}
